        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-4">
          ...
          <div class="flex justify-end space-x-4">
            <button type="button" (click)="close()" 
                    class="px-4 py-2 text-sm font-normal text-gray-600 hover:text-gray-800 transition-colors duration-300">
              Cancel
            </button>
            <button type="submit" [disabled]="form.invalid" 
                    class="px-4 py-2 bg-blue-500 text-white rounded-md text-sm font-normal hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed">
              Save
            </button>
          </div>
        </form>
